...
from model import BertClassifier,AttBertClassifier,BertLstmClassifier,BertLstmAttClassifier,BertAVGClassifier

parser = argparse.ArgumentParser()
parser.add_argument('--max_length', type=int, default=128, help='the input length for bert')
parser.add_argument('--batch_size', type=int, default=128)
parser.add_argument('--lda_matrix', type=str, default='topic_distribution_matrix.csv')
parser.add_argument('--attention', type=int, default=4,
                    choices=[0,1,2,3,4], help='0表示原始状态, 1表示使用attention, 2表示使用lstm, 3表示使用lstm+attention, 4表示avg')
...
if checkpoint_dir is None:
    ckpt_dir = '../checkpoint/{}_{}_{}'.format(bert_init, dataset, str(attention))
else:
...
if attention == 1:
    model = AttBertClassifier(pretrained_model="llms/"+bert_init, nb_class=nb_class)
elif attention == 0:
    model = BertClassifier(pretrained_model="llms/"+bert_init, nb_class=nb_class)
elif attention == 2:
    model = BertLstmClassifier(pretrained_model="llms/"+bert_init, nb_class=nb_class)
elif attention == 3:
    model = BertLstmAttClassifier(pretrained_model="llms/" + bert_init, nb_class=nb_class)
elif attention == 4:
    model = BertAVGClassifier(pretrained_model="llms/" + bert_init, nb_class=nb_class)
