...
parser.add_argument('-m', '--m', type=float, default=0.7, help='Weight factor between BERT and GCN')
parser.add_argument('--nb_epochs', type=int, default=50)
parser.add_argument('--bert_init', type=str, default='roberta-base',
                    choices=['roberta-base', 'roberta-large', 'bert-base-uncased', 'bert-large-uncased', 'deberta-v3-base'])
...
parser.add_argument('--n_hidden', type=int, default=200, help='Hidden dimension for GCN')
...
# ------------------------ Logger Setup ------------------------
ckpt_dir = args.checkpoint_dir or f'./checkpoint/{args.bert_init}_{args.gcn_model}_{args.dataset}'
...
if args.gcn_model == 'gcn':
    ...
    model = BertGAT(nb_class=nb_class, pretrained_model=args.bert_init, m=args.m,
                    gcn_layers=args.gcn_layers, heads=args.heads, n_hidden=args.n_hidden, dropout=args.dropout)
...
input_ids = th.cat([input_ids[:-nb_test], th.zeros((nb_word, args.max_length), dtype=th.long), input_ids[-nb_test:]])
attention_mask = th.cat([attention_mask[:-nb_test], th.zeros((nb_word, args.max_length), dtype=th.long), attention_mask[-nb_test:]])
...
def train_step(engine, batch):
    ...
    with th.no_grad():
        if train_mask_batch.sum() > 0:
            train_acc = accuracy_score(y_true.cpu(), y_pred.argmax(dim=1).cpu())
        else:
            train_acc = 1
